...
import com.te.true2java.stream.PipeLine4.Employee.Manager;
...
public class PipeLine4 {
	static class Employee {
		...
		static class Manager {
			...
			public Manager(Integer id, String firstName, Float salary, Integer age, Integer experience) {
				super();
				this.id = id;
				this.firstName = firstName;
				this.salary = salary;
			...
			@Override
			public String toString() {
				return "Manager [id=" + id + ", firstName=" + firstName + ", salary=" + salary + ", age=" + age
						+ ", experience=" + experience + "]";
			}
	...
	public static void main(String[] args) {
		List<Employee> employees = new ArrayList<PipeLine4.Employee>();
		...
		List<Manager> collectedData = employees.stream()
				.filter(e -> e.getExperience() > 5)
				.map(e -> new Manager(e.getId(), e.getFirstName(), e.getSalary(), e.getAge(), e.getExperience()))
