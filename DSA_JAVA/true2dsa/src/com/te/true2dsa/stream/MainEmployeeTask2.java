...
public class MainEmployeeTask2 {
	...
	public static void main(String[] args) {
		...
		List<Integer> collectedDate = employees.stream()
				.filter(e -> !(e.getDateOfJoining().isAfter(LocalDate.now().minusYears(5))
						&& e.getPerformanceRating() < 8 &&(e.getSalary() >= 60000 && e.getSalary() <= 120000)))
				.sorted(Comparator.comparing(Employee::getSalary)
						.thenComparing(Employee::getYearsOfExprience).reversed())
				.map(e->e.getEmployeeId())
				.collect(Collectors.toList());


		System.err.println(collectedDate.size());
		System.out.println("==============After===============");
		collectedDate.forEach(System.out::println);
	}
